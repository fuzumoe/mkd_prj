...
class LatestAnalysisHistoryView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if user_email:
            return AnalysisHistory.objects.filter(user_email=user_email).order_by('-created_at')[:1]
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            if check_password(password, user.password):
                return Response(RegisteredUserSerializer(user).data, 
...
def upload_profile_image(request):
    ...
    if not email or not profile_image:
        return Response({'error': 'Email and profile image are required.'}, 
                       status=status.HTTP_400_BAD_REQUEST)
...
def change_password(request):
    ...
    if old_password == new_password:
        return Response({'error': 'New password must be different.'}, 
                      status=status.HTTP_400_BAD_REQUEST)

    if len(new_password) < 8:
        return Response({'error': 'Password must be ≥8 characters.'}, 
                      status=status.HTTP_400_BAD_REQUEST)
...
def analysis_summary(request, email):
    ...
    issue_counts = {}
    for h in history:
        issue_counts[h.predicted_condition] = issue_counts.get(h.predicted_condition, 0) + 1

    common_issue = max(issue_counts, key=issue_counts.get)
    return Response({
        'last_analysis': last.created_at.strftime('%Y-%m-%d'),
        'common_issue': common_issue,
        'skin_score': round(last.confidence * 100) if last.confidence is not None else None
    })
...
def recommended_products(request, email):
    ...
    def score(product):
        ...
        targets_text = ", ".join(product.targets) if isinstance(product.targets, list) else str(product.targets or '')
        suitable_text = ", ".join(product.suitable_for) if isinstance(product.suitable_for, list) else str(product.suitable_for or '')
...
def user_activity(request, email):
    ...
    analysis = AnalysisHistory.objects.filter(user_email=email).order_by('-created_at')[:1]
    if analysis.exists():
    ...
    consult = ConsultationRequest.objects.filter(email=email).order_by('-submitted_at')[:1]

    if consult.exists():
        activity.append({ 'description': f"Consultation booked with {getattr(consult[0], 'doctor_name', 'a specialist')}" })
...
def user_consultations(request, email):
    consultations = ConsultationRequest.objects.filter(email=email).order_by('-submitted_at')
